    ...
    navigate('/payments', { 
      state: { 
        product: {
          ...
          productType: 'shop' // To distinguish from other types of purchases
        } 
      } 
    });
  };
...
const handleDelete = async (productId) => {
  ...
  const startChat = async (userId, companyName) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
      ...
      // First create a chat with the shop provider
      ...
      if (response.data.success && response.data.chatId) {
        // Navigate to the chat room with the new chat ID
            ...
            <>
              ...
              {user?.role === 'client' && (
                <div className="flex justify-center mt-3 mb-4">
                  <button
                    className="bg-gray-800 hover:bg-gray-700 text-white px-5 py-2 rounded-lg font-medium transition-all duration-300 flex items-center justify-center"
                    onClick={() => startChat(products[0].userId, products[0].shopName)}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd" />
                    </svg>
                    Chat with {products[0].shopName}
                  </button>
                </div>
              )}
            </>
